...
//insuring to preappend the checker value...
    public void printLabelings(Tree T, InstanceList testInstances, FileWriter[] fos, int index, String abstractOutName, String abstractName, int depth) throws IOException {
	...
	for(int tree = 0; tree < NUMTREES; tree ++){
		...
		for (int k = 0; k < FOLDS; k ++){	
			...
			for(int i = 0; i < 300; i ++){
        		...
        		ImportExample4 importer = new ImportExample4();
			importer.foldNum = k;
		...
		newargs[4] = importer.cat5(newargs[0], newargs[1], newargs[2], newargs[3]);
			...
			for(int index = 0; index < testInstances.size(); index++){
				importer.printLabelings(importer.T, testInstances, fos, index, FileNames[index], AbstractNames[index], 0);
			}

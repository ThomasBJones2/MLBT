...
void score(int TreeNum) throws IOException{
	...
	for(int i = 0; i < SMterminator.length; i ++){
		...
		SMNum[TreeNum - 24][k][SMterminator.length] += SMNum[TreeNum][k][i];
		SMDen[TreeNum - 24][k][SMterminator.length] += SMDen[TreeNum][k][i];
	...
	for(int i = 0; i < STterminator.length; i ++){
		...
		if(TreeNum < 24){
			...
			STNum[TreeNum-24][k][STterminator.length] += STNum[TreeNum][k][i];
			STDen[TreeNum-24][k][STterminator.length] += STDen[TreeNum][k][i];
	...
	for(int i = 0; i < RMterminator.length; i ++){
		...
		if(TreeNum < 48){
			...
			RMNum[TreeNum - 24][k][RMterminator.length] += RMNum[TreeNum][k][i];
			RMDen[TreeNum - 24][k][RMterminator.length] += RMDen[TreeNum][k][i];
		}
	}
	if(k == FOLDS-1 && TreeNum == NUMTREES-1){
		double out = 0;
	...
	for(int i = 0; i < RTterminator.length; i ++){
		...
		if(TreeNum < 72){	
			...
			RTNum[TreeNum - 24][k][RTterminator.length] += RTNum[TreeNum][k][i];
			RTDen[TreeNum - 24][k][RTterminator.length] += RTDen[TreeNum][k][i];
		}
	}
	if(k == FOLDS-1 && TreeNum == NUMTREES-1){
		double out = 0;
